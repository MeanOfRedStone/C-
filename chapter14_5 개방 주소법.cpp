...
Quiz. 아래와 같이 10개의 버켓과 버켓 당 1개의 슬롯을 가지는 해시 테이블이 있다. 
해시 함수는 정수 데이터의 끝자리 숫자를 사용하고, 선형 조사법으로 오버플로우를 처리한다.
데이터(74, 30, 12, 28, 24, 33, 52, 60, 10, 20)가 순서대로 삽입될 경우에 해시 테이블에 데이터가 저장되는 과정을 보여라.
0 30
1 60
2 12
3 33
